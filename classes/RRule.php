...
//    eZDebug::writeDebug( "RRule", " Text %s from epoch %d", $this->_text, $this->_epoch );
  }
...
//    eZDebug::writeDebug( "RRule", " Epoch %d from %04d-%02d-%02d %02d:%02d:%02d", $this->_epoch, $this->_yy, $this->_mo, $this->_dd, $this->_hh, $this->_mi, $this->_ss );
  }
  ...
  */
  function DaysInMonth( $mo=false, $yy=false ) {
    ...
    switch( $mo ) {
      ...
      default:
        eZDebug::writeDebug( "ERROR"," Invalid month of '%s' passed to DaysInMonth", $mo );
        break;
...
//    eZDebug::writeDebug( "RRule", " Adding %d months to %s", $mo, $this->_text );
    $this->_mo += $mo;
    while ( $this->_mo < 1 ) {
      $this->_mo += 12;
      $this->_yy--;
    ...
    $this->_EpochFromParts();
    $this->_TextFromEpoch();
//    eZDebug::writeDebug( "RRule", " Added %d months and got %s", $mo, $this->_text );
  }
  ...
  */
  function AddDays( $dd ) {
    ...
    $this->_EpochFromParts();
    $this->_TextFromEpoch();
//    eZDebug::writeDebug( "RRule", " Added %d days to %s and got %s", $dd, $at_start, $this->_text );
  }
  ...
  */
  function AddDuration( $duration ) {
    ...
    $sign = ( $sign == "-" ? -1 : 1);
//    eZDebug::writeDebug( "RRule", " Adding duration to '%s' of sign: %d,  days: %s,  time: %s", $this->_text, $sign, $days, $time );
...
//    eZDebug::writeDebug( "RRule", " Adding %02d:%02d:%02d * %d to %02d:%02d:%02d", $hh, $mi, $ss, $sign, $this->_hh, $this->_mi, $this->_ss );
    $this->_hh += ($hh * $sign);
    $this->_mi += ($mi * $sign);
    $this->_ss += ($ss * $sign);
...
//    eZDebug::writeDebug( "RRule", " Testing BYMONTH %s against month %d", (isset($monthlist) ? $monthlist : "no month list"), $this->_mo );
...
//    eZDebug::writeDebug( "RRule", " Applying BYDAY %s to month", $byday );
...
//    eZDebug::writeDebug( "RRule", " Applying BYMONTHDAY %s to month", $bymonthday );
    $days_in_month = $this->DaysInMonth();
...
//    eZDebug::writeDebug( "RRule", " MonthDays: Looking at %d for first match on (%s/%s), %d for numeric", $first_matching_day, $matches[1], $matches[2], $numeric );
...
//    eZDebug::writeDebug( "RRule", " ApplyBySetPos: Applying set position '%s' to set of %d days", $bysplist, count($set) );
    $subset = array();
...
class RRule {
  ...
  */
  function RRule( $start, $rrule ) {
    ...
    eZDebug::writeDebug( "RRule", " new RRule: Start: %s, RRULE: %s", $start->Render(), $this->_rule );

    $parts = explode(';',$this->_rule);
    $this->_part = array( 'INTERVAL' => 1 );
    foreach( $parts AS $k => $v ) {
      list( $type, $value ) = explode( '=', $v, 2);
//      eZDebug::writeDebug( "RRule", " Parts of %s explode into %s and %s", $v, $type, $value );
      $this->_part[$type] = $value;
    }

    // A little bit of validation
    if ( !isset($this->_part['FREQ']) ) {
      eZDebug::writeDebug( "ERROR", " RRULE MUST have FREQ=value (%s)", $rrule );
    }
    if ( isset($this->_part['COUNT']) && isset($this->_part['UNTIL'])  ) {
      eZDebug::writeDebug( "ERROR", " RRULE MUST NOT have both COUNT=value and UNTIL=value (%s)", $rrule );
    }
    if ( isset($this->_part['COUNT']) && intval($this->_part['COUNT']) < 1 ) {
      eZDebug::writeDebug( "ERROR", " RRULE MUST NOT have both COUNT=value and UNTIL=value (%s)", $rrule );
    }
    if ( !preg_match( '/(YEAR|MONTH|WEEK|DAI)LY/', $this->_part['FREQ']) ) {
      eZDebug::writeDebug( "ERROR", " RRULE Only FREQ=(YEARLY|MONTHLY|WEEKLY|DAILY) are supported at present (%s)", $rrule );
    }
...
//    eZDebug::writeDebug( "RRule", " WithinScope: Processing list of %d days relative to %s", count($relative_days), $base->Render() );
    foreach( $relative_days AS $day => $v ) {

      $test = new iCalDate($base);
      $days_in_month = $test->DaysInMonth();

//      eZDebug::writeDebug( "RRule", " WithinScope: Testing for day %d based on %s, with %d days in month", $day, $test->Render(), $days_in_month );
...
//      eZDebug::writeDebug( "RRule", " WithinScope: Testing if %s is within scope", count($relative_days), $test->Render() );
...
//        eZDebug::writeDebug( "RRule", " WithinScope: Looks like %s is within scope", $test->Render() );
        $ok_days[$day] = $test;
        $ptr++;
      }
...
//        eZDebug::writeDebug( "RRule", " GetNext: Returning %s, (%d'th)", $this->_dates[$this->_current]->Render(), $this->_current );
...
//      eZDebug::writeDebug( "RRule", " GetNext: Calculating more dates for MONTHLY rule" );
      $limit = 200;
      do {
        $limit--;
        do {
...
//      eZDebug::writeDebug( "RRule", " GetNext: Found %d days for MONTHLY rule", count($days) );

    }
    else if ( $this->_part['FREQ'] == "WEEKLY" ) {
//      eZDebug::writeDebug( "RRule", " GetNext: Calculating more dates for WEEKLY rule" );
      $limit = 200;
      do {
        $limit--;
        if ( $this->_started ) {
...
//      eZDebug::writeDebug( "RRule", " GetNext: Found %d days for WEEKLY rule", count($days) );
    }
    else if ( $this->_part['FREQ'] == "DAILY" ) {
//      eZDebug::writeDebug( "RRule", " GetNext: Calculating more dates for DAILY rule" );
      $limit = 100;
      do {
        $limit--;
        if ( $this->_started ) {
...
//      eZDebug::writeDebug( "RRule", " GetNext: Found %d days for DAILY rule", count($days) );
    }
...
//      eZDebug::writeDebug( "RRule", " GetNext: Returning null date" );
      $next = null;
      return $next;
    }
  }
